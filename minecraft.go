...
func (a *App) downloadFile(url string, destPath string, reportProgress bool) error {
	...
	if reportProgress {
		...
		if err != nil { return fmt.Errorf("写入文件失败 %s: %v", destPath, err) }
	}
	return nil
}
...
func (a *App) GetVersionManifest() ([]MCVersion, error) {
	...
	for _, u := range urls {
		...
		defer resp.Body.Close()
		if resp.StatusCode != http.StatusOK { lastErr = fmt.Errorf("HTTP %d from %s", resp.StatusCode, u); continue }
		body, err = io.ReadAll(resp.Body)
		if err != nil { lastErr = err; continue }
		lastErr = nil
		break
	}
	if lastErr != nil { return nil, fmt.Errorf("获取版本清单失败: %v", lastErr) }
...
func (a *App) GetInstalledVersions() ([]InstalledVersionInfo, error) {
	...
	if err != nil {
		if os.IsNotExist(err) { return []InstalledVersionInfo{}, nil }
	...
	for _, entry := range entries {
		...
		if !a.validateVersionInstallation(versionFolder) { continue }
...
func (a *App) scanVersionFolder(versionFolder string, folderName string) InstalledVersionInfo {
	...
	if _, err := os.Stat(jsonPath); err != nil {
		...
		if len(jsonFiles) > 0 { jsonPath = jsonFiles[0] }
	}
	...
	if gameVersion == "" { gameVersion = versionJSON.ID }
	versionType := "game"
	if loader != "" { versionType = "loader" }
...
func (a *App) AddToDownloadList(versionID string, versionURL string, customName string, versionType string) error {
	...
	if customName == "" { return fmt.Errorf("版本名称无效") }
	for _, item := range a.downloadList {
		if item.CustomName == customName { return fmt.Errorf("下载列表中已存在同名版本: %s", customName) }
	}
...
func (a *App) AddToDownloadListWithLoader(versionID string, versionURL string, customName string, versionType string, loaderName string, loaderVersion string, optifineType string, optifinePatch string) error {
	...
	if loaderName != "" { displayName += " (" + loaderName + " " + loaderVersion + ")" }
	a.downloadList = append(a.downloadList, DownloadItem{
...
func (a *App) AddJavaToDownloadList(majorVer int) error {
	...
	for i := range javaList {
		if javaList[i].MajorVer == majorVer { target = &javaList[i]; break }
	}
	if target == nil { return fmt.Errorf("未找到 Java %d 的下载信息", majorVer) }
	a.downloadMutex.Lock()
	defer a.downloadMutex.Unlock()
	customName := "Java " + fmt.Sprintf("%d", majorVer)
	for _, item := range a.downloadList {
		if item.CustomName == customName { return fmt.Errorf("下载列表中已存在: %s", customName) }
	}
	a.downloadList = append(a.downloadList, DownloadItem{
...
func (a *App) StartDownloadList() error {
	a.downloadMutex.Lock()
	if a.isDownloading { a.downloadMutex.Unlock(); return fmt.Errorf("正在下载中，请等待当前下载完成") }
	if len(a.downloadList) == 0 { a.downloadMutex.Unlock(); return fmt.Errorf("下载列表为空") }
	...
	go func() {
		...
		for {
			...
			case <-a.downloadCancel:
				a.downloadMutex.Lock()
				for i := range a.downloadList {
					if a.downloadList[i].Status == "pending" { a.downloadList[i].Status = "cancelled" }
				}
				a.downloadMutex.Unlock()
				runtime.EventsEmit(a.ctx, "downloadListUpdated", a.GetDownloadList())
				return
			...
			if item == nil { a.downloadMutex.Unlock(); break }
			...
			if item.ItemType == "java" {
				...
				if err = a.DownloadVersion(item.ID, item.URL, item.ID); err != nil {
					...
					case "forge": err = a.InstallForge(item.ID, item.LoaderVersion)
					case "fabric": err = a.InstallFabric(item.ID, item.LoaderVersion)
					case "neoforge": err = a.InstallNeoForge(item.ID, item.LoaderVersion)
					case "optifine": err = a.InstallOptiFine(item.ID, item.OptiFineType, item.OptiFinePatch)
					}
					if err != nil { err = fmt.Errorf("安装加载器失败: %v", err) }
				}
			} else {
...
func (a *App) CancelDownloadList() error {
	a.downloadMutex.Lock()
	defer a.downloadMutex.Unlock()
	if !a.isDownloading || a.downloadCancel == nil { return fmt.Errorf("当前没有正在进行的下载") }
...
func (a *App) downloadJavaItem(majorVer int, url string) error {
	...
	if tempDir == "" { tempDir = filepath.Join(os.Getenv("USERPROFILE"), "AppData", "Local", "Temp") }
	destPath := filepath.Join(tempDir, target.FileName)
	if _, err := os.Stat(destPath); err == nil { return runInstaller(destPath, target.IsMSI) }
	a.emitProgress("downloading", target.FileName, 0, 0)
	resp, err := safeHTTPClient().Get(url)
	if err != nil { return fmt.Errorf("下载失败: %v", err) }
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK { return fmt.Errorf("下载失败: HTTP %d", resp.StatusCode) }
	out, err := os.OpenFile(destPath, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0600)
...
func runInstaller(filePath string, isMSI bool) error {
	...
	if isMSI {
		if ext != ".msi" { return fmt.Errorf("不安全的文件类型: %s", ext) }
		cmd := exec.Command("msiexec", "/i", filePath)
		cmd.SysProcAttr = &syscall.SysProcAttr{HideWindow: true}
		return cmd.Start()
	}
	if ext != ".exe" { return fmt.Errorf("不安全的文件类型: %s", ext) }
...
func (a *App) DownloadVersion(versionID string, versionURL string, customName string) error {
	...
	if err != nil { return fmt.Errorf("读取版本 JSON 失败: %v", err) }
	...
	for _, lib := range versionJSON.Libraries {
		...
		if lib.Natives != nil {
			...
			if lib.Downloads.Classifiers == nil { continue }
			classifier, ok := lib.Downloads.Classifiers[nativeKey]
			if !ok || classifier == nil { continue }
...
func (a *App) checkFileHash(filePath string, expectedSHA1 string) bool {
	...
	if err != nil || info.Size() == 0 { return false }
...
func (a *App) fixAssetsIndex(mcDir string, versionJSON *VersionJSON) {
	if versionJSON.AssetIndex == nil || versionJSON.AssetIndex.URL == "" { return }
	assetIndexDir := filepath.Join(mcDir, "assets", "indexes")
	assetIndexPath := filepath.Join(assetIndexDir, versionJSON.AssetIndex.ID+".json")
	if _, err := os.Stat(assetIndexPath); err == nil { return }
...
func (a *App) fixMissingAssets(mcDir string, assetIndexPath string) {
	...
	if err := json.Unmarshal(data, &index); err != nil { return }
	...
	for _, obj := range index.Objects {
		...
		if _, err := os.Stat(objPath); err == nil { continue }
		missingCount++
		...
		if a.downloadFromMirrors(urls, objPath) { downloadedCount++ }
	}
	if missingCount > 0 {
...
func mavenNameToURL(name string) string {
	...
	classifier := ""
	if len(parts) >= 4 { classifier = "-" + parts[3] }
...
func extractNatives(jarPath string, destDir string) (int, error) {
	...
	for _, f := range r.File {
		if f.FileInfo().IsDir() || strings.HasPrefix(f.Name, "META-INF/") { continue }
		lowerName := strings.ToLower(f.Name)
		if !strings.HasSuffix(lowerName, ".dll") && !strings.HasSuffix(lowerName, ".jnilib") && !strings.HasSuffix(lowerName, ".so") { continue }
		rc, err := f.Open()
		if err != nil { continue }
		...
		if _, err := os.Stat(destPath); err == nil { os.Remove(destPath) }
		out, err := os.OpenFile(destPath, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0600)
...
func (a *App) extractNativesFromJSON(mcDir string, nativesDir string, versionJSON *VersionJSON) {
	...
	for _, lib := range versionJSON.Libraries {
		...
		if lib.Downloads == nil || lib.Downloads.Classifiers == nil { continue }
		classifier, ok := lib.Downloads.Classifiers[nativeKey]
		if !ok || classifier == nil { continue }
...
func (a *App) setGameLanguage(gameDir string, versionID string) {
	...
	if len(parts) > 0 {
		...
		if len(verParts) >= 2 {
			if v, err := strconv.Atoi(verParts[1]); err == nil { mcCodeMain = v }
		}
	}
	requiredLang := "zh_cn"
	if mcCodeMain > 0 && mcCodeMain < 12 { requiredLang = "zh_CN" }
	...
	if info, err := os.Stat(filepath.Join(gameDir, "saves")); err == nil && info.IsDir() { hasSaves = true }
	for i, line := range lines {
		line = strings.TrimSpace(line)
		if strings.HasPrefix(line, "lang:") {
			currentLang := strings.TrimSpace(strings.TrimPrefix(line, "lang:"))
			if currentLang == requiredLang { a.writeLog("游戏语言已为 %s，无需修改", requiredLang); return }
	...
	if !found { lines = append(lines, fmt.Sprintf("lang:%s", requiredLang)) }
	...
	for _, ruleRaw := range rules {
		...
		if features, hasFeatures := rule["features"]; hasFeatures {
			if featMap, ok := features.(map[string]interface{}); ok {
				if _, isDemo := featMap["is_demo_user"]; isDemo { return false }
			}
		}
		...
		if action == "allow" && !matchesOS { return false }
		if action == "disallow" && matchesOS { return false }
	}
	return true
}
...
func deduplicateJvmArgs(args []string) []string {
	...
	for _, arg := range args {
		if strings.HasPrefix(arg, "-") {
			if current.fullKey != "" { groups = append(groups, current) }
...
func findMinecraftWindow(targetPID int, processStartTime time.Time) (bool, bool) {
	...
	cb := syscall.NewCallback(func(hwnd uintptr, lParam uintptr) uintptr {
		...
		if !isMCWindow { return 1 }
		title := make([]byte, 512)
		procGetWindowText.Call(hwnd, uintptr(unsafe.Pointer(&title[0])), 512)
		titleStr := string(bytesToString(title))
		if titleStr == "" || titleStr == "PopupMessageWindow" { return 1 }
		if strings.HasPrefix(titleStr, "GLFW") && !strings.HasPrefix(titleStr, "FML") { return 1 }
		var windowPID int
		procGetWindowPID.Call(hwnd, uintptr(unsafe.Pointer(&windowPID)))
		if windowPID != targetPID { return 1 }
		found = true
...
func (a *App) shouldIncludeLib(lib Library) bool {
	if len(lib.Rules) == 0 { return true }
	allow := false
	for _, rule := range lib.Rules {
		osMatch := true
		if rule.OS != nil { osMatch = rule.OS.Name == "windows" }
...
func (a *App) resolveVersionJSON(versionID string) (*VersionJSON, error) {
	...
	jsonData, err := os.ReadFile(jsonPath)
	if err != nil { return nil, fmt.Errorf("未找到版本 JSON: %s", versionDir) }
	var versionJSON VersionJSON
	if err := json.Unmarshal(jsonData, &versionJSON); err != nil {
...
func (a *App) mergeVersionJSON(parent *VersionJSON, child *VersionJSON) VersionJSON {
	...
	if child.ReleaseTime != "" { merged.ReleaseTime = child.ReleaseTime }
	if child.Jar != "" { merged.Jar = child.Jar }
	childGroupArtifacts := map[string]bool{}
	for _, lib := range child.Libraries { childGroupArtifacts[libGroupArtifact(lib.Name)] = true }
	var mergedLibs []Library
	for _, lib := range parent.Libraries {
		if !childGroupArtifacts[libGroupArtifact(lib.Name)] { mergedLibs = append(mergedLibs, lib) }
	}
	mergedLibs = append(mergedLibs, child.Libraries...)
	merged.Libraries = mergedLibs
	if child.MinecraftArgs != "" { merged.MinecraftArgs = child.MinecraftArgs }
	if child.Arguments != nil {
		if merged.Arguments == nil { merged.Arguments = &ArgumentsObj{} }
		if len(child.Arguments.JVM) > 0 { merged.Arguments.JVM = append(merged.Arguments.JVM, child.Arguments.JVM...) }
		if len(child.Arguments.Game) > 0 { merged.Arguments.Game = append(merged.Arguments.Game, child.Arguments.Game...) }
	}
	if child.Downloads != nil { merged.Downloads = child.Downloads }
	if child.AssetIndex != nil { merged.AssetIndex = child.AssetIndex }
...
func (a *App) buildClasspath(mcDir string, versionID string, versionJSON *VersionJSON) []string {
	...
	for _, lib := range versionJSON.Libraries {
		if !a.shouldIncludeLib(lib) || lib.Natives != nil { continue }
		var libPath string
	...
	if jarName == "" { jarName = versionJSON.InheritsFrom }
	if jarName == "" { jarName = versionID }
	...
	if _, err := os.Stat(versionJar); err != nil {
		...
		if _, err2 := os.Stat(altJar); err2 == nil { versionJar = altJar }
	}
